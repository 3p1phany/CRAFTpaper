...
The disparity among these scenarios motivates row buffer management policies. An *open-page* policy keeps the row buffer open after an access. A *close-page* policy precharges the bank immediately. Neither  static policy dominates universally, as shown in Figure 1.
...
ABP [Awasthi+11] maintains a per-row access counter to predict the likelihood of future reaccesses to each row. This approach requires approximately 20 KB of storage per channel in the form of a set-associative prediction table. This cost is impractical for modern DDR5 controllers with 32 banks per channel.

DYMPL [Rafique+22] employs a perceptron to combine seven extracted features and make page policy decisions for every memory request. A 512-entry page row table accounts for 86.6% of the 3.39 KB per-channel storage. The perceptron learns empirical correlations among features. It does not directly observe the cost consequences of its precharge decisions.

INTAP [[INTEL+06]] adjusts timeout values per bank through a mistake counter with a fixed step size. At approximately 200 B per channel, INTAP achieves the lowest storage overhead among the three baselines. Its adjustment mechanism, however, is symmetric. A wrong precharge and a conflict trigger equal step magnitudes in opposite directions. This treatment does not distinguish between the entirely avoidable overhead of a wrong precharge and the inherent cost of a conflict.

These scheme-specific limitations differ in nature. All three baselines, however, share a deeper deficiency. None of them distinguishes precharge outcomes by the nature of the waste they produce. Timeout-based speculative precharge exists to preserve row buffer locality. A wrong precharge directly negates this benefit. Both the precharge command and the subsequent reactivation are entirely unnecessary. Neither operation would occur under the open-page baseline. The total avoidable overhead is tRP + tRCD. A conflict involves a request to a different row. The precharge and activation are inherent to the access pattern. A correctly timed precharge would have overlapped tRP with idle bank cycles. Only this overlap opportunity is avoidable. The avoidable component is therefore tRP. Under DDR5-4800 timing with tRP = tRCD = 40 cycles, the ratio of controller-attributable waste is (tRP + tRCD) / tRP = 2:1. This ratio has direct implications for timeout adaptation. The escalation step for wrong precharges should exceed the de-escalation step for conflicts. None of the three baselines incorporates this cost asymmetry into its adaptation logic.
...
CRAFT exploits the qualitatively different nature of cost among these outcomes to derive both the direction and the magnitude of each adjustment.
...
Timeout-based speculative precharge serves both sides of the row buffer management trade-off. Keeping a row open allows same-row accesses to be served as row buffer hits. Issuing a timely precharge overlaps precharge overhead with idle bank cycles, reducing the effective penalty for subsequent different-row accesses. A wrong precharge eliminates the first benefit entirely. Both the precharge command and the subsequent reactivation are unnecessary operations that would not occur under the open-page baseline. The total controller-attributable waste therefore amounts to tRP + tRCD.

Conflicts present a fundamentally different cost model. The target row differs from the buffered row, so the precharge and subsequent activation are inherent to the access pattern regardless of the timeout decision. Only the timing constitutes a controller-attributable error. A correctly timed precharge would have overlapped tRP with idle bank cycles. The avoidable overhead is therefore limited to the missed overlap opportunity of tRP.

The ratio of avoidable overhead between these two outcomes is (tRP + tRCD) / tRP. Under our DDR5-4800 configuration with tRP = tRCD = 40 cycles, this ratio evaluates to 2:1. CRAFT mirrors this asymmetry in its adjustment magnitudes. The escalation step for wrong precharges uses a base value of BASE_STEP = 50 cycles. This value is slightly larger than tRP, so each escalation produces an observable change in the timeout. The de-escalation step for conflicts is scaled by the factor tRP / (tRP + tRCD) and evaluates to 25 cycles under the same configuration.

This asymmetry creates a deliberate upward bias. Under equal frequencies of wrong precharges and conflicts, each escalation exceeds the subsequent de-escalation and produces a net timeout increase. This bias reflects the cost asymmetry between the two outcomes. A wrong precharge incurs tRP + tRCD of avoidable overhead, twice the missed-overlap cost of a conflict. A stronger corrective response to wrong precharges is therefore proportional to the actual cost difference. T_MAX bounds this upward bias. Once the timeout reaches 3200 cycles, further escalation is clamped. Conflicts and right-streak de-escalation (Section 3.2a) then provide downward pressure to restore balance.
...
The reopen_streak counter tracks consecutive wrong precharges, is capped at 8 and resets to zero upon a right precharge or a conflict. This mechanism enables rapid convergence during sustained high-locality phases. The timeout can reach the upper bound within a small number of feedback events.

**(c) Continuous adjustment range.**
CRAFT adjusts the timeout continuously within a bounded range [T_MIN, T_MAX] = [50, 3200] cycles. T_MIN is set to match BASE_STEP, preventing degeneration into a pure close-page policy. T_MAX is set to accommodate sustained high-locality phases and prevents the row buffer from remaining indefinitely open. The feedback loop can therefore converge to any intermediate value for fine-grained adaptation to varying degrees of row locality. CRAFT's exponential backoff traverses the full timeout range in as few as six escalation events.
...
After four or more consecutive right precharges, the timeout decrements by half the conflict de-escalation step.
...
precharges induced by read requests and write requests differ in performance impact. A wrong precharge followed by read requests directly stalls the processor pipeline. The delayed read request  blocks retirement of dependent instructions. A  wrong precharge which blocks write commands is absorbed by the write buffer and incurs minimal pipeline impact.
Across our evaluated workloads, read operations account for the majority of wrong precharge events. CRAFT exploits this asymmetry by halving the escalation step for write wrong precharges and doubling the de-escalation step for read conflicts. RW requires no additional storage. The command type is already available to the memory controller.

**(c) Streak Decay (SD).**
Each right precharge decrements the reopen_streak counter by one, allowing the escalation magnitude to decay gradually rather than persisting at a historically elevated level.
