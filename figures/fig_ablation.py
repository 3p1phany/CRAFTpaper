...
for i, (v, r, d) in enumerate(zip(variants, ratios, deltas)):
    if i == 0:
        ax.text(i, 101.5, '100%', ha='center', va='bottom',
                fontsize=FONT_ANNOT, fontweight='bold', color=COLORS_DARK['craft'])
    elif d > 0.5:
        ax.text(i, r + 1.5, f'+{d:.1f}%', ha='center', va='bottom',
                fontsize=FONT_ANNOT, fontweight='bold', color=delta_dark[i])
    elif d < -0.5:
...
cat_y = -0.17
ax.text(0, cat_y, 'Core Loop', ha='center', fontsize=FONT_ANNOT,
        color=COLORS_DARK['craft'], transform=ax.get_xaxis_transform())
ax.text(2, cat_y, 'Precharge-path', ha='center', fontsize=FONT_ANNOT,
        color=COLORS_DARK['open_page'], transform=ax.get_xaxis_transform())
ax.text(4.5, cat_y, 'Conflict-path', ha='center', fontsize=FONT_ANNOT,
        color=COLORS_DARK['closed_page'], transform=ax.get_xaxis_transform())
ax.text(6.5, cat_y, 'Combined', ha='center', fontsize=FONT_ANNOT,
...
for i, tl in enumerate(ax.get_xticklabels()):
    if variants[i] in ('BASE', 'PRECHARGE', 'ALL'):
